...
(
    Material(vec3(1.0, 0.1, 0.1), 0.05, 0.8, 0.2, 0.3, 0.3),
    Material(vec3(0.1, 1.0, 0.1), 0.05, 0.8, 0.2, 0.3, 0.3),
    Material(vec3(0.1, 0.1, 1.0), 0.0, 0.6, 0.1, 0.8, 0.8),
    Material(vec3(1.0), 1.0, 0.5, 0.5, 0.5, 0.5),
    Material(vec3(1.0, 1.0, 0.1), 0.01, 0.5, 0.1, 0.5, 0.5),
    Material(vec3(1.0, 0.1, 1.0), 0.01, 0.5, 0.1, 0.5, 0.5),
    Material(vec3(1.0, 0.5, 0.1), 0.0, 0.5, 0.1, 0.5, 0.5)
);
...
const int p_light_num = 7;
const PointLight p_light_list[] = PointLight[]
(
    PointLight(vec3(0.0, 1.0, 0.0), 0.25, vec3(1.0, 1.0, 1.0), 150.0),
    PointLight(vec3(-8.0, 1.0, 0.0), 0.125, vec3(1.0, 1.0, 1.0), 75.0),
    PointLight(vec3(-8.0, 1.0, 8.0), 0.125, vec3(1.0, 1.0, 1.0), 75.0),
    PointLight(vec3(8.0, 1.0, 0.0), 0.125, vec3(1.0, 1.0, 1.0), 75.0),
    PointLight(vec3(8.0, 1.0, 8.0), 0.125, vec3(1.0, 1.0, 1.0), 75.0),
    PointLight(vec3(8.0, 8.0, -10.0), 0.125, vec3(0.0, 1.0, 1.0), 75.0),
    PointLight(vec3(-8.0, 8.0, -10.0), 0.125, vec3(1.0, 0.0, 1.0), 75.0)
);

const int d_light_num = 0;
...
(
    Sphere(vec3(-3.0, -1.0, -1.0), 1.0, material_list[0]),
    Sphere(vec3(3.0, -1.0, -1.0), 1.0, material_list[1])
);
...
vec3 calcDiffSpec(Material material, vec3 N, vec3 L, vec3 V, vec3 F0, vec3 diffuse_reflectance, vec3 light_intensity)
{
    ...
    else
    {
        ...
        float HdotL = dot(H, L);
        vec3 sheen = material.sheen_strength * mix(vec3(1.0), tint, material.sheen_tint) * pow(1 - HdotL, 5.0);
        diffuse += sheen;
    }
